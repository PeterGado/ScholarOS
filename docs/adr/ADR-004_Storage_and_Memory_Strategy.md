...
1. SQLite's write concurrency limit is a known ceiling; acceptable until multi-user stages (07 §5 stage 2) trigger the PostgreSQL migration.
   **Refinement, 2026-09-16 (Frontend milestone real manual workflow):** this ceiling is reachable even within a single user's single session, not only at the multi-user stage this item anticipated - a real browser frontend routinely fires several authenticated requests in parallel (e.g., two `useQuery` hooks resolving on the same page), and every authenticated request also writes (`AuthSessionRepository.touch`'s activity timestamp, committed per request). Two such requests arriving genuinely concurrently can race for SQLite's single writer slot. Three targeted mitigations were applied (`app/database/session.py`, `app/auth/service.py`): (a) explicit transaction demarcation (SQLAlchemy's documented pysqlite `isolation_level=None` + explicit `BEGIN` workaround) - found necessary first, to fix a *correctness* bug (a pooled connection could read a stale, pre-commit snapshot) rather than for concurrency itself, but a prerequisite for the app's transaction boundaries to mean anything precise; (b) `PRAGMA busy_timeout` and WAL journal mode, standard SQLite configuration for reducing writer-vs-writer contention; (c) making the non-critical session-activity write specifically resilient to a transient lock (already documented as "must not affect whether a session is considered valid" - the *failure handling* just hadn't matched that yet). These reduce, but by design do not eliminate, the underlying ceiling - a genuine business write (e.g., a document upload) can still occasionally lose a lock race under real concurrent load. Full elimination requires either a serialized-write strategy (a single writer queue in front of SQLite) or the PostgreSQL migration this item already names, not further SQLite configuration; reported here rather than pursued further under time pressure, per this project's standing "report genuine architectural gaps rather than silently patch them" discipline. See `docs/Frontend_Implementation_Plan.md` §9 for the full investigation.
