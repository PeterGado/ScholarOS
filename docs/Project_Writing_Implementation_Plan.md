...
**Status:** Backend complete and validated — Stages 2-8 done, and both of Stage 8's own findings have since been resolved (2026-09-16, by explicit project-owner instruction). Stage 6 was reviewed on 2026-09-15 (two real defects found and fixed - a Work Item payload-size limitation and a Context Assembly truncation-ordering bug); Stage 7 exposed the full Writing Service over authenticated HTTP (closing a genuine retrieval-integration gap and a small pre-existing Review persistence gap); Stage 8 validated the entire loop end to end against a fresh environment with the real, already-configured Gemini provider, found no defect requiring a fix, and surfaced two findings — an imprecise crash-window claim in the Stage 7 report, and an unresolved conflict between an explicit 2026-09-12 decision (§19 item 3) and what Stage 7 actually shipped. **Both are now resolved:** a general, kind-agnostic stale-`RUNNING` Work Item recovery mechanism was added to `app/workers/` (§18 Stage 8's finding, resolution recorded there), and `Conversation`/`Message`/`Message Context Link` were implemented as originally decided, with generation instructions now genuinely persisted through them (§19 item 3's resolution recorded there). Full regression: 524/524. All six original open decisions in §19 were resolved by explicit user approval on 2026-09-12 — see §19 for the recorded decisions. The backend has been committed; a Frontend milestone is the next authorized phase (see `docs/Frontend_Implementation_Plan.md` once started).
...
### Stage 3 — Writing-Style Ingestion & Extraction — Complete

**Objective:** realize `Writing Profile`, `Profile Characteristic`, `Profile Characteristic Source`; build the style-document → characteristic extraction pipeline (mirrors Stage 4-5's mechanical-then-semantic split). Delivered as two sub-stages, gated separately: ingestion (accept and persist a style sample) then extraction (derive real characteristics from it via the AI provider) - narrower per-step than this entry originally described, and reported as a deliberate split rather than silently expanded scope.
**Files:** `app/modules/writing/{domain,application,infrastructure,interface}/*` (style sub-area): `style_ingestion.py`, `style_extraction.py` (domain + application), plus the two routes below.
**New entities:** the three style entities (§13) - `Writing Profile`, `Profile Characteristic`, `Profile Characteristic Source` - realized exactly as frozen, no invented fields.
**New APIs:** `POST /writing/style-profile/documents` (ingestion - reuses `Research Document`/`ContentStore`, auto-creates the Agent's Writing Profile on first upload, never enqueues a Knowledge Processing Pipeline Work Item) and `POST /writing/style-profile/extract` (extraction - reuses the existing `TextGenerationProvider`, no new AI abstraction).
...
**Behavior:** preserves input evidence ordering, removes duplicate or blank chunks, applies a maximum evidence count, includes source metadata, labels style signals as guidance rather than facts, includes current memory, and truncates deterministically within a character budget with sentence-boundary preference. The grounding-rules section's budget is reserved before the other sections are fit, so it always survives truncation instead of being silently dropped when long evidence content fills the budget first - a real defect found during review and fixed before Stage 7 (the earlier version rendered it last and let it be the first thing dropped under any tight budget, exactly when the reminder not to fabricate unsupported claims mattered most).
**Tests:** 9 focused unit tests covering determinism, evidence deduplication, limits, style/memory inclusion, truncation, grounding-rules survival under evidence pressure, and invalid input.
...
### Stage 6 — Async Generation — Complete (reviewed and corrected)
...
**Work Item contract:** `kind=pipeline_stage`; `payload_reference` is `generate_draft_version:<draft_id>:<request_id>:<context_store_reference>` - a genuine short pointer, the same "Short Text - reference to the work payload" shape as `process_document:<id>` and `ResearchDocument.content_reference` (04_Logical_Data_Model.md §3.20, §3.4). `idempotency_key` is `generate_draft_version:<request_id>`.
**A real defect found and fixed before Stage 7:** the first version of this stage serialized the *entire* `ContextAssemblyInput` (including full evidence chunk content) directly into `payload_reference` and rejected anything over 512 characters - workable only for near-empty contexts, since Context Assembly's own default budget is 12,000 characters across up to 10 evidence chunks. No test caught it because every test used a single trivial one-line evidence string. Fixed by writing the serialized context to the existing `ContentStore` (the same object-store category `UploadResearchDocumentUseCase` already writes into, ADR-004 §5) and embedding only the resulting short reference in `payload_reference`, exactly mirroring how `ResearchDocument.content_reference` already works. A regression test now proves a realistic, evidence-sized context (10 chunks, ~2000 characters each) enqueues successfully with a payload reference under 200 characters.
...
**Execution:** the executor parses the payload (reading the context back from the content store) and invokes `GenerateDraftVersionUseCase`; it does not duplicate generation or persistence logic. Stage 5 commits DraftVersion plus DraftEvidenceLink rows atomically before the Work Item is marked succeeded.
...
**Tests:** real SQLite worker tests, now using realistic evidence-sized content, prove queued/claimed execution, DraftVersion/evidence persistence, persisted success and terminal-failure states, retry behavior, no partial generation persistence, and unchanged document-worker behavior.
**Acceptance:** verified for the implemented flow and retry semantics, including realistic-sized requests. Known limitation, unchanged: if the process crashes after Stage 5 commits but before the Work Item success marker commits, a retry can create another DraftVersion; the current frozen schema has no generation-request reference to close that crash window. Resolving that would require a later authorized schema/design decision.
**Non-goals:** no HTTP API, frontend, new queue, new Work Item kind, or provider abstraction.

### Stage 7 — API — Complete

**Objective:** expose the writing capabilities as thin, authenticated HTTP routes over the now-complete Writing Service.
**Files:** `app/modules/writing/interface/{routes.py,schemas.py}`, `app/core/dependencies.py`, `app/api/exception_handlers.py`, plus three new application-layer files: `application/generation_request.py` (`RequestDraftGenerationUseCase`), `application/reviews.py` (`SubmitDraftReviewUseCase`), `application/profile_view.py` (`GetWritingProfileUseCase`).
**New entities:** none - zero schema changes.
**New APIs:** `POST/GET /writing/drafts`, `GET /writing/drafts/{draft_id}`, `POST /writing/drafts/{draft_id}/generate`, `GET /writing/drafts/{draft_id}/versions`, `POST /writing/drafts/{draft_id}/versions/{version_id}/reviews` (this path shape, from §14, was followed over the Stage 7 engineering prompt's own simpler illustrative example - the plan is the authoritative, already-approved source), `GET /writing/style-profile`.
**A genuine gap found during reconnaissance, not a Stage 6 defect:** Stage 5/6 both deliberately take an already-resolved `ContextAssemblyInput` as a parameter; nothing before Stage 7 ever built the piece that resolves one from a bare HTTP "generate my draft" request. Exposing the existing enqueue use case directly to a route that accepted evidence from the client would have let a caller fabricate arbitrary Draft Evidence Link content. Resolved by adding `RequestDraftGenerationUseCase`, which composes the existing `SearchKnowledgeUseCase` (Slice 2 Stage 8), the Agent's Writing Profile/Memory Records, and the existing, unmodified `EnqueueDraftGenerationUseCase` - no new architectural mechanism, no Stage 6 redesign.
**A second, smaller gap found and fixed:** `SqlAlchemyReviewRepository.add()` (Stage 2) never wrote `decided_at` to the database at all, since Stage 2 never needed to create an already-decided Review. Fixed by passing it through and re-syncing it from the row after flush, the same way `opened_at` already was.
**Ownership:** every route resolves the Agent from the authenticated `user_id` (`get_current_user_id`); no route accepts `agent_id` or `user_id` from the client. `CreateDraftUseCase` (Stage 2) was extended to resolve `agent_id` from `user_id` internally, its first real caller beyond tests.
**Draft status transitions:** review outcomes `approved`/`revisions_requested` transition Draft status per the frozen lifecycle table; `rejected` has no documented transition and deliberately leaves Draft status unchanged (same position taken in the Stage 2 report for this exact ambiguity - not re-decided here).
**Tests:** 50 new tests - 20 unit (fakes), 8 integration (real SQLite, including a regression test for the `decided_at` fix), 22 e2e (real HTTP via TestClient, including the full enqueue → Work Item → separate worker step → evidence-linked version loop, and explicit cross-user ownership tests for every new endpoint).
**Acceptance:** complete - the full create → generate → review loop works over real HTTP plus a separate worker step; verified again over a genuine real-`uvicorn` process (health, auth enforcement, ownership enforcement, empty-list and not-found paths, and the existing `ProviderConfigurationError` → 503 contract inherited with zero new code).
**Non-goals:** no frontend, no PostgreSQL/pgvector/Alembic, no new queue/worker mechanism, no new AI provider abstraction.

### Stage 8 — Full Validation — Complete
...
**Files:** none (validation only) - no code changed, no defect required a fix.
**What was done:** full regression (509/509, unchanged from Stage 7); a genuinely fresh SQLite DB + filesystem storage root (isolated `/tmp` environment, never the dev DB) exercised over real `uvicorn`: login/logout/token-invalidation, single-user provisioning survives restart with no duplicates, Agent/Project creation, a real research document processed end-to-end by the real Gemini provider (extraction + embedding, verified against real Knowledge Element/Chunk/Chunk Evidence Link/Embedding rows), real-embedding-based `GET /knowledge/search`, Draft creation, **two real generation requests** through `POST /writing/drafts/{id}/generate` (real query embedding for retrieval, real Gemini text generation for drafting, both processed automatically by the app's own background executor - no manual work-item injection was needed), immutable versioning confirmed (version 1 unchanged after version 2 was created), a review submission with `decided_at` persisted correctly, a real writing-style extraction (5 genuine characteristics from a real sample), and a full evidence-provenance trace from `DraftVersion` through `DraftEvidenceLink` → `KnowledgeChunk` → `ChunkEvidenceLink` → `ResearchDocument` → the actual stored source bytes. A full stop/restart of the application confirmed zero duplication of any row (user/agent/project/work item/draft/version/evidence/profile all exactly as before). Database integrity audit (FK check, orphan checks across every writing table) came back clean.
**A Stage 7 report claim corrected by investigation, not accepted at face value:** the "crash-window... a retry can create another DraftVersion" wording is imprecise. `WorkItemRepository.claim_next_queued()` only ever claims `state=QUEUED` rows; nothing anywhere resets a `RUNNING` row back to `QUEUED`. A crash between `GenerateDraftVersionUseCase`'s own commit and the executor's subsequent `mark_succeeded`+commit leaves the Work Item permanently stuck in `RUNNING`, never automatically retried - not "duplicated by an automatic retry." The only path to a second version is a fresh, user-initiated `POST .../generate` call, which is normal, intended behavior, not a bug. This is also not new to Project Writing: it is a general, pre-existing ADR-006/Slice-2 Work Item characteristic (no "stuck RUNNING" recovery exists for the document pipeline either). Recorded as a corrected, more precise characterization of the same already-deferred gap, not resolved.
**RESOLVED 2026-09-16, as a general ADR-006/worker concern, not a Writing-specific fix (per explicit project-owner instruction).** `WorkItemRepository.requeue_stale_running(stale_after)` (`app/workers/repository.py`) recovers any Work Item — of either `kind`, Writing or Knowledge Processing — left stuck in `RUNNING` past a threshold (default 10 minutes), reusing the existing bounded-retry rule (`mark_failed`'s own attempts-vs-`max_attempts` logic) so a poison item still terminates in `failed` rather than looping. `WorkItemExecutorLoop.start()` (`app/workers/executor.py`) calls it once, synchronously, before polling begins — never mid-poll, so it can never race the item the current process is itself still working on. Covered by new integration tests, including a full `WorkItemExecutorLoop` start-to-completion run. Full regression re-run clean (524/524).
**A second genuine conflict found and reported, not silently resolved:** §19 item 3's 2026-09-12 decision ("instructions stored via a Draft-scoped Conversation/Message") was never implemented - Stage 7 shipped a direct `instructions` request field instead (Conversation/Message remains deferred since Stage 2). Flagged in §19 item 3 directly, not overwritten.
**Acceptance:** the complete path — Agent → topic → style samples → research documents → knowledge → **writing request → context assembly → real generation → evidence-linked draft → review** — proven end to end with the real, already-configured Gemini provider (two tiny real generations, one real style extraction; never a paid requirement, well within the free tier).
**Non-goals:** no new functionality; no code was changed - this stage found no defect requiring a fix within its own scope.
...
3. **Per-request writing instructions.** ~~No field exists on `Draft`/`Draft Version`~~ (§13). **DECIDED: Option (b).** Instructions are stored as the first `Message` of a Draft-scoped `Conversation` (via `Message Context Link`, already frozen — §4.4), not as a new column. No correction ADR. Stage 2 must include wiring a Draft to its Conversation (see §13 update below) and Stage 4 (Context Assembly) must read the instruction from that Message rather than from a new field.
   **Stage 8 finding — this decision was never actually implemented and the repository contradicts it.** Stage 2 deliberately deferred Conversation/Message (recorded, reported, not silently dropped — see Stage 2's own report). Stage 7's `RequestDraftGenerationUseCase`/`RequestDraftGenerationRequest` instead accept `instructions` as a direct request field, never reading from a Conversation/Message. This is a genuine, real conflict between an explicit 2026-09-12 decision and the shipped Stage 7 code, surfaced during Stage 8 validation rather than hidden. Not resolved here — building Conversation/Message remains explicitly out of scope for Stage 8 (validation-only). Flagged for an explicit project-owner decision: either re-approve the direct-field approach as the MVP's actual mechanism (superseding this line), or schedule Conversation/Message construction as a prerequisite before any further Writing API work.
   **RESOLVED 2026-09-16, by explicit project-owner instruction ("Resolve the instructions contract discrepancy").** The original decision (Option (b)) was implemented as originally specified, not superseded: `Conversation`, `Message`, and `Message Context Link` now exist as real domain entities, ORM models, and repositories (`app/modules/writing/domain/entities.py`, `infrastructure/models.py`, `infrastructure/repositories.py`). `RequestDraftGenerationUseCase` still accepts `instructions` as a plain HTTP request field (`RequestDraftGenerationRequest` is unchanged — no client-facing contract break), but now finds-or-creates a Draft-scoped Conversation (via the free-form `title` field, keyed `draft:<id>:instructions` — deliberately not a new `draft_id` column, so the frozen Conversation shape is untouched) and persists the instructions as that Conversation's next `Message` before enqueueing generation. The Work Item's `payload_reference` now carries the resulting `message_id` end to end; once generation succeeds, the executor (`app/workers/executor.py`) links that instructions Message — and a new system-response Message recording the version produced — to the resulting `Draft Version` via `Message Context Link`. `MemoryProvenanceLink.conversation_id` also gained the real foreign key it was missing (Conversation now exists to reference). Covered by new domain/unit/integration tests; full regression re-run clean (524/524).
...
Stage 2 → Stage 3 → Stage 4 → Stage 5 → Stage 6 → Stage 7 → Stage 8, exactly as numbered in §18 — each stage is a strict prerequisite for the next. All eight stages are now implemented and validated. Stage 6 was reviewed on 2026-09-15 and two real defects (payload-size limitation, grounding-rules truncation ordering) were found and fixed. Stage 7 exposed the full Writing Service over HTTP, closing a genuine retrieval-integration gap (`RequestDraftGenerationUseCase`) and a small pre-existing Review persistence gap (`decided_at` never written). Stage 8 validated the complete loop against a fresh environment with the real Gemini provider, found zero code defects, corrected the crash-window gap's mechanism (no automatic retry occurs at all for a `RUNNING`-stuck Work Item - a pre-existing, general ADR-006 characteristic, not Writing-specific), and surfaced that §19 item 3's Conversation/Message decision was never actually implemented. **Project Writing's backend is now validated end to end. The next milestone (whatever it is - API refinement, the deferred items below, or a new phase such as frontend integration) requires explicit authorization**, per this project's standing stage-gate discipline.
