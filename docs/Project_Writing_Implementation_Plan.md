# Project Writing — Stage 1 Implementation Plan

**Status:** Complete (planning only — no code written, no ADR created, no frozen document modified). All six open decisions in §19 were resolved by explicit user approval on 2026-09-12 — see §19 for the recorded decisions.
...
**Objective:** the pure Context Assembly function (§6) — topic + user instructions (read from the Draft's Conversation, §19.3) + retrieved knowledge + style + memory → one assembled prompt, applying the general-AI-knowledge policy (§19.5: retrieved knowledge is the primary grounding source; general knowledge only for non-factual scaffolding). No provider call yet in this stage; test the assembly logic itself.
**Files:** `app/modules/writing/domain/context_assembly.py` (or `application/`, decided at implementation time based on whether it needs repository access — likely `application/` since it calls `SearchKnowledgeUseCase`).
**New entities:** none.
**New APIs:** none.
**Dependencies:** `SearchKnowledgeUseCase` (Stage 8, unmodified), Writing Profile/Memory repositories (Stage 2/3), the Draft's Conversation/Message (Stage 2).
**Tests:** unit (deterministic assembly given fixed inputs), integration (real retrieval + real style data).
**Acceptance:** given a Draft, produces a complete, correctly-prioritized, correctly-truncated prompt with a matching list of "chunks used" for later evidence-linking.
**Non-goals:** no LLM call yet — this stage is provable without one.
...
Stage 2 → Stage 3 → Stage 4 → Stage 5 → Stage 6 → Stage 7 → Stage 8, exactly as numbered in §18 — each stage is a strict prerequisite for the next (persistence before context assembly, context assembly before generation, generation before async wiring, async wiring before API, API before end-to-end validation). No stage should be reordered or merged; this mirrors the discipline every Slice 2 stage already followed successfully. All six items in §19 are now resolved (2026-09-12); Stage 2 is unblocked pending explicit authorization to begin implementation, per this project's standing stage-gate discipline.
